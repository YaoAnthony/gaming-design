...
物品栏里的「Boss 大史莱姆」物件（`K`）放进哪个房间，那个房间就是 Boss 战。玩家一进来：两侧入口封上岩石、顶部出 10 格血条、大史莱姆从你放物件的位置落下。它的循环是：游走两三跳（离玩家太近往外跳、太远才靠近）→ 蓄力 0.7 秒（压扁变暗）→ 扑向玩家当时的位置、落地重震 → 喘气 1.6 秒并吐 2 只小史莱姆（上限 6）。喘气那段是砸它的窗口。碰到即死。封门在玩家离开门口一格半以后才落下。**只有落石和引线能伤它**：快速下落的碎块砸中扣最多 4 格并被它吞掉，引线烧过的格子碰到它扣 2 格；受击后 0.8 秒无敌（闪烁），所以一次落石、一次引线各只算一下；起跳爆炸不伤它。血条清空后开门。R / 死亡重置后 Boss 重新出现。参数在 `config.bossHp / bossHopMs / bossSpitMs / bossMaxMinions`。
