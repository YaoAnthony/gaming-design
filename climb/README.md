...
第一层是塔外，走进「塔门」物件（`T`）进下一层，正式开始。编辑器画布上方是层标签，「添加一层」可以给每层单独的房间尺寸（比如标题层做成 40×20）。层顺序 = 标签顺序；双击改名，右键删除。
...
物品栏里的「Boss 大史莱姆」物件（`K`）放进哪个房间，那个房间就是 Boss 战。玩家进来先没事；往里走到离门口一格半以上，两侧入口封上岩石，复活点就定在这个位置，随后顶部出 5 格血条、大史莱姆从你放物件的位置落下。它的循环是：游走两三跳（离玩家太近往外跳、太远才靠近）→ 蓄力 0.7 秒（压扁变暗）→ 扑向玩家当时的位置、落地重震 → 喘气 1.6 秒并吐 2 只小史莱姆（上限 6）。喘气那段是砸它的窗口。碰到即死。**只有落石和引线能伤它**：快速下落的碎块砸中扣最多 4 格并被它吞掉，引线烧过的格子碰到它扣 2 格；受击后 0.8 秒无敌（闪烁），所以一次落石、一次引线各只算一下；起跳爆炸不伤它。血条清空后开门，同时 Boss 爆开射出一圈**穿墙火花**（`bossBurstCount / Speed / Ttl`）：不伤任何人，碰到引线端点就点燃。把引线埋在 Boss 房地板下、连到别处的门，打赢 Boss 就自动开路。死掉或按 R 都回到关门的位置重来，门重新关、Boss 重新出场。参数在 `config.bossHp / bossHopMs / bossSpitMs / bossMaxMinions`。
