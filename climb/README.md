...
第一层是塔外，走进「小城堡」物件（`T`，城门在贴图底部正中）进下一层，正式开始。进门有一段旋涡：镜头转着拉近城门，人、怪、骷髅和一把碎屑沿螺旋线缩进门里，然后淡出换层。编辑器画布上方是层标签，「添加一层」可以给每层单独的房间尺寸（比如标题层做成 40×20）。层顺序 = 标签顺序；双击改名，右键删除。
...
「骷髅」物件（`N`）是挡在路上的小角色。走到它跟前一格半内强制进入对话（左右移动锁住，只能跳），每跳一次（也就是每炸一次）说下一句；最后一句说完再跳，它带着笑声（`bossLaugh`）淡出消失，路就通了。消失是这一层内永久的，死亡重置不会把它带回来。台词在 `game/registry/dialogues.ts`，新角色 = 一段台词 + 一个 `defineEntity`（`host.addNpc`）。放它的时候脚下用岩石，不然玩家跳的那几下会把它脚下炸空。
