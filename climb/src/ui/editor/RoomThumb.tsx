...
interface Props { rows: string[]; entities?: string[]; fuse?: string[]; roomW: number; roomH: number; scale?: number }

const hex = (c: number) => '#' + c.toString(16).padStart(6, '0');

/** 房间缩略图：每格一个色块，颜色来自注册表 */
export function RoomThumb({ rows, entities, fuse, roomW, roomH, scale = 3 }: Props) {
  ...
  }, [rows, entities, fuse, roomW, roomH, scale]);
